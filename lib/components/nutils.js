...
nutils.addMic = function (device, data, dir) {
    ...
    data = Buffer.concat([data, (new Buffer([cmac[0], cmac[1], cmac[2], cmac[3]]))]);
...
nutils.downlinkCheck = function (device, data) {
    ...
    if (data.mhdr.mType === CNST.MType.JOINACCEPT) {
        ...
        if (data.mhdr.mType === CNST.MType.JOINREQUEST | data.mhdr.mType === CNST.MType.UNCONFIRMDATAUP | data.mhdr.mType === CNST.MType.CONFIRMDATAUP)
            return;
...
nutils.decryptPayload = function (device, data) {
    ...
    if (FPort === null) {
        ...
        // decrypt FRMPayload
        // Payload = nutils.frmPayloadcrypto(device, dir, Payload, device.info.nwkSKey);
    } else {    // Payload is application data, [TODO] use different interface? lwm2m...
        // Payload = nutils.frmPayloadcrypto(device, dir, Payload, device.info.appSKey);
    }
    data.macPayload.payload = Payload;
    return data;
};
